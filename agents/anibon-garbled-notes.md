...
description: >
  Post-timestamp cleanup subagent for the anibon-timestamper pipeline. Reads the
  GARBLED_NOTES blocks emitted by chunk timestampers, cross-checks them against the
  raw transcript, writes garbled_notes.json to the workspace, and appends confirmed
  corrections to the shared garbled_replacements.json dictionary. Use once after all
  anibon-chunk-timestamper subagents have returned and timestamps are merged.
---
