...
```json
{
  ...
  "notes": [
    {
      "garbled": "GamePlaystyle่ะ",
      "google_sentence": "<Google ASR surrounding sentence>",
      "whisper_segment": "<Whisper.cpp time-aligned acoustic sentence>",
      "correct": "Gameplay",
      "chunk": "chunk_07",
      "ts": "00:33:26",
      "cluster_span": "00:33:19-00:33:34"
    }
  ]
}
```
...
Each entry contains `garbled`, `google_sentence` (Google ASR sentence context), `whisper_segment` (Whisper.cpp acoustic ground truth), `correct`, `chunk`, `ts`, and `cluster_span`.
Deduplicate identical words and reject false positives (standard loanwords like `FGO`, `NP`, `YouTube`, `AI`).

### Step 2: Sentence-Level & Word-by-Word Sequence Alignment
Each note in `garbled_notes.json` provides:
- `garbled`: The phonetic noise/hybrid token spotted by chunk timestampers.
- `google_sentence`: The exact sentence window transcribed by Google ASR from YouTube captions.
- `whisper_segment`: The exact time-aligned acoustic sentence segment from `whisper.cpp`.
- `correct`: Pre-resolved canonical word (or `null` awaiting alignment).

Align the spoken sentence word-by-word:
1. Locate `garbled` within `google_sentence` and identify its immediate preceding and following Thai anchor words.
2. Locate the corresponding anchor words in `whisper_segment` (the same spoken sentence from the speaker).
3. The spoken word occupying the matching slot in `whisper_segment` is the acoustic ground-truth replacement.
4. Match this candidate against known game titles, character names, Japanese loanwords, and English slang.
5. When confirmed: assign `correct: "<CanonicalTarget>"`.
6. If audio is silent/inconclusive or ambiguous: leave `correct: null` (strict zero-hallucination policy).
...
- If `/Users/zenithth/abss-dev` exists, also run `python3 /Users/zenithth/abss-dev/skills/cleaning-auto-transcripts/scripts/update_garbled_dictionary.py --from-notes <workspace>/garbled_notes.json` to ensure the git tracking repository stays synchronized.
