...
**REQUIRED (subagent execution):** Load `batching-subagents-concurrency` when running multi-subagent analysis (max 10 per batch turn with `Model: "flash"`). Ask user permission before downloading large external media or performing long web fetches.
