...
def main():
    ...
    parser.add_argument("-g", "--gap-limit", type=int, default=600,
                        help="Seconds before a gap triggers a warning")
    parser.add_argument("-w", "--warn-bytes", type=int, default=3500,
                        help="Byte threshold for YouTube block warnings")
    parser.add_argument("-v", "--verbose", action="store_true",
                        help="Show matched keywords per chunk")

    args = parser.parse_args()

    try:
        chunks = load_chunks_list(args.workspace_dir)
    ...
    if gaps:
        for gap in gaps:
            sys.stderr.write(f"⚠  GAP DETECTED: {gap['from_ts']} → {gap['to_ts']}  ({gap['gap_sec']}s, limit={args.gap_limit}s)\n")
        sys.stderr.write("\n")
    ...
    for block in blocks:
        status = block["status"]
        icon = "✅ OK   " if status == "OK" else "⚠  WARN" if status == "WARN" else "❌ OVER "
