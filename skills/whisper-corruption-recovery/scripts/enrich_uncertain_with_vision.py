...
def extract_frames(json_path: Path, video_path: Path, out_dir: Path) -> list:
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(json_path, "r", encoding="utf-8", errors="replace") as f:
        data = json.load(f)

    uncertain_items = [x for x in data if x.get("uncertain") or "[?]" in x.get("text", "")]
    print(f"[vision] Found {len(uncertain_items)} uncertain [?] items.")

    extracted = []
    for idx, item in enumerate(uncertain_items):
